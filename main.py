...
datefmt ='%Y-%m-%d %H:%M:%S'
logging.basicConfig(level=level, format=format, filename=log_filename, datefmt=datefmt)

if __name__ == '__main__':
    view_list = ["%03d" % x for x in range(0, 181, 18)]
    train_dir = ["nm-%02d" % i for i in range(1, 5)]
    train_dir.append("cl-02")
    val_dir = ["cl-01"]
    ...
    for train_view in view_list:
        val_view = train_view
        training_x, training_y, validation_x, validation_y = load_training_validation_data(train_view=train_view, val_view=val_view, train_dir=train_dir, val_dir=val_dir)
